...
public class GameHandler {
	...
	boolean isGameOver() {//승리조건에 만족했는지 판단
		...
		//+ 오직 5개만 연속했를 때 승리조건에 부합하도록,
		//+ 바둑판에 돌이 다 채워졌을 때 비겼다는 메시지를 출력할 수 있도록한다. 
		//+ 바둑판이 꽉차지 않은 상태에서 어디에 돌을 놓아도 
		//  승리할 수 없는 상태가 되면 마찬가지로 비겼다는 메시지를 출력할 수 있도록 한다.
		
		//5개가 연속되면 해당 턴의 돌이 승리했다는 메시지가 출력되도록 했다.
